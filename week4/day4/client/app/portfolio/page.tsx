...
export default function PortfolioPage() {
  ...
  const saveToDb = async (newHoldings: Holding[]) => {
    const token = localStorage.getItem('token');
    if (token) {
      // Save to server
      const holdingsToSave = newHoldings.map(({ currentPrice, ...h }) => h);
      try {
        await api.post('/portfolio', { holdings: holdingsToSave });
      } catch (error) {
        console.error('Failed to save portfolio to DB:', error);
      }
    }
    // Save to localStorage
    localStorage.setItem('cryptoPortfolio', JSON.stringify(newHoldings));
  };

  const handleAddAsset = async (transaction: Transaction) => {
    let updatedHoldings: Holding[] = [];
    const prev = Array.isArray(holdings) ? holdings : [];
    ...
    await saveToDb(updatedHoldings);
  };

  const handleRemoveAsset = async (symbol: string) => {
    const prev = Array.isArray(holdings) ? holdings : [];
    const updatedHoldings = prev.filter(h => h.symbol !== symbol);
    setHoldings(updatedHoldings);
    await saveToDb(updatedHoldings);
  };
  ...
  const portfolioStats = useMemo(() => {
    if (!Array.isArray(holdings)) return { totalValue: 0, totalCost: 0, totalPnL: 0, totalPnLPercent: 0 };

    const totalValue = holdings.reduce((sum, holding) => {
      const priceStr = prices[holding.symbol];
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {paginatedData.map((holding, index) => {
                        ...
                        const valuationPrice = currentPrice ?? holding.avgPrice;
                        const currentValue = holding.amount * valuationPrice;
