      ...
      <div className="flex flex-col gap-3">
        {fields.map((f, i) => (
          <div key={f.id} className="relative group">
            <input
              {...register(`certifications.${i}`)}
              maxLength={100}
              className={`w-full px-4 py-2.5 bg-gray-50 border rounded-lg transition-all outline-none 
                ${errors?.certifications?.[i] ? "border-red-500 focus:ring-red-200" : "border-gray-200 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent"}
              `}
            ...
            {errors?.certifications?.[i] && (
              <p className="text-red-500 text-xs mt-1 absolute -bottom-4 left-1">
                {errors.certifications[i]?.message}
              </p>
            )}
          </div>
        ))}
